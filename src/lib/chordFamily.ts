/**
 * "Family chords" (a.k.a. diatonic chords): the 7 chords built on each
 * degree of a major or natural-minor scale, and a handful of common chord
 * progressions expressed generically in scale-degree terms so they can be
 * resolved against any key.
 *
 * Pure logic only — no rendering. Every resolved chord comes from the
 * seeded chord library (`data/chords`) via its accessors, never fabricated
 * here; see the `chord: Chord | undefined` note on `FamilyDegree` for the
 * one case where a degree currently has no library entry to resolve to.
 */
import { findChordsByRoot } from '../data/chords'
import type { Chord, NoteName } from '../data/chords/types'

/** The two diatonic scale types this module builds families for. */
export type ScaleType = 'major' | 'minor'

/**
 * The triad quality built on a given scale degree. The seeded chord
 * library (`data/chords`) doesn't include diminished shapes, so
 * `'diminished'` degrees never resolve to a `Chord` (see `FamilyDegree`).
 */
...
export interface FamilyDegree {
  ...
  /**
   * The resolved chord from the chord library, looked up by root + quality
   * via `findChordsByRoot`. `undefined` when the library has no voicing for
   * this root/quality — currently only the diminished (°) degree, since no
   * diminished shapes are seeded yet.
   */
...
/**
 * Resolves a degree's root + quality to a real `Chord` from the library.
 * Diminished degrees are never resolved (no diminished quality is seeded).
 */
function resolveDegreeChord(
  root: NoteName,
  quality: DegreeQuality,
): Chord | undefined {
  if (quality === 'diminished') return undefined
 ...
 * `Chord` from the library where one is seeded.
 */
