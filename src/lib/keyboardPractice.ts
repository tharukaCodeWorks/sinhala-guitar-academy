/**
 * Pure logic + localStorage persistence helpers for the keyboard Technique
 * Drills page (chord-change practice, the keyboard-track counterpart to the
 * guitar Fingering Practice drill). Kept framework-free so the trickier bits
 ...
 *
 * Deliberately a reimplementation of `lib/fingeringPractice.ts` rather than a
 * re-export of it: same math/shape, but with its own `PRACTICE_INTERVALS_SECONDS`,
 * a keyboard-specific `STORAGE_KEY` (`'sga.keyboardTechnique.sessions.v1'`,
 * distinct from the guitar module's `'sga.fingeringPractice.sessions.v1'`) so
 * guitar and keyboard drill history never mix in one list, and its own
 * `sessionIdCounter`/prefix so ids from the two modules can't collide either.
 *
 * `pages/KeyboardTechniquePage.tsx` is the only consumer; it owns the
...
const STORAGE_KEY = 'sga.keyboardTechnique.sessions.v1'
 ...
 * millisecond still get distinct ids). Prefixed distinctly from the guitar
 * module's `session-` ids so the two are never mistakable for each other.
 */
export function createSessionId(): string {
  sessionIdCounter += 1
  return `kb-session-${Date.now()}-${sessionIdCounter}`
}
