...
import { getSongById } from '../data/keyboardSongs'
import {
  getLessonById,
  nextLessonInTier,
  previousLessonInTier,
} from '../data/lessons'
import type { Lesson, LessonSection, LessonTier } from '../data/lessons/types'
...
/**
 * Lesson Detail page: one lesson's full teaching content, section by
 * section, with a `KeyboardDiagram` embedded wherever a section references
 * a chord, plus Previous/Next navigation within the lesson's tier (via the
  ...
  return (
    <section className="mx-auto w-full max-w-3xl flex-1 px-6 py-12">
      ...
      >
        {previousLesson ? (
          <Link
            to={`/keyboard/lessons/${previousLesson.id}`}
        ...
        {nextLesson ? (
          <Link
            to={`/keyboard/lessons/${nextLesson.id}`}
...
function LessonSectionBlock({ section }: LessonSectionBlockProps) {
  const chord = section.keyboardChordId
    ? getChordById(section.keyboardChordId)
    : undefined
