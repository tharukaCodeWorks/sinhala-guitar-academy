...
import { chordLibrary, listQualities, listRoots } from '../data/chords'
 ...
 * (`src/data/chords`), each rendered with the `ChordDiagram` component.
 * Clicking a chord opens a detail view of all of its seeded variants.
 */
function ChordLibraryPage() {
  ...
  const selectedChord: Chord | undefined = useMemo(
    () => chordLibrary.find((chord) => chord.id === selectedChordId),
    [selectedChordId],
  )
    ...
    <section className="mx-auto w-full max-w-6xl flex-1 px-6 py-12">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
          Browse every chord shape in the library. Click a chord to see all of
          its fingerings.
        </p>
      </div>
...
/** Modal overlay listing every seeded variant/shape for a single chord. */
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="chord-detail-heading"
        ...
        <div className="flex flex-wrap justify-center gap-8">
          {chord.variants.map((variant) => (
            <div key={variant.id} className="flex flex-col items-center gap-2">
              <ChordDiagram variant={variant} size="lg" />
