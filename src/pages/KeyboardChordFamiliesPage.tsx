...
import { listRoots } from '../data/keyboardChords'
import type { NoteName } from '../data/keyboardChords/types'
import { findSongsByChordId } from '../data/keyboardSongs'
 ...
 * real keyboard voicings via `lib/keyboardChordFamily.ts` — plus a handful
 * of common chord progressions built from those same 7 chords. Each family
 * chord also surfaces "heard in" callouts pointing to real songs from
 * `data/keyboardSongs` whose arrangements use it, so a learner can jump
 * straight from theory to a real piece. Structurally mirrors the guitar
 * `ChordFamiliesPage`.
 */
function KeyboardChordFamiliesPage() {
    ...
    <section className="mx-auto w-full max-w-6xl flex-1 px-6 py-12">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
          ...
          together. Pick a key to see its family, some progressions built from
          it, and real songs that use those chords.
        </p>
      </div>
...
/**
 * One family-chord card: the diagram (when a shape exists) plus its
 * roman-numeral label and any "heard in" real-song callouts.
 */
function FamilyDegreeCard({ degree }: FamilyDegreeCardProps) {
  ...
  const songs: Song[] = useMemo(
    () => (chord ? findSongsByChordId(chord.id) : []),
    [chord],
  )
  ...
  return (
    <div className="flex h-full flex-col items-center gap-2 rounded-lg border border-gray-200 p-4 text-center dark:border-gray-800">
      ...
      {songs.length > 0 && (
        <div className="mt-1 flex flex-col items-center gap-0.5 border-t border-gray-100 pt-1.5 dark:border-gray-800">
          <span className="text-[10px] font-semibold tracking-wide text-gray-400 uppercase dark:text-gray-600">
            Heard in
          </span>
...
export default KeyboardChordFamiliesPage
